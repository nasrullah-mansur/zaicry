...
<div class="col-lg-12">
    <div class="card">
        ...
        <div class="card-body card-block">
            <form action="{{isset($users) ? route('user.update', $users->id) : route('user.store')}}" method="POST" enctype="multipart/form-data">
                @csrf
                @if(isset($users))
                @method('PUT')
                @endif
                ...
                <div class="row form-group">
                    ...
                    <div class="col-12 col-md-9">
                        <select name="role" id="role" class="form-control ">
                            ...
                            <option value="admin" @if(isset($users)) @if($users->role == 'admin')
                                selected
                                @endif
                                @endif
                                >Admin</option>
                            <option value="user" @if(isset($users)) @if($users->role == 'user')
                                selected
                                @endif
                                @endif
                                >User</option>
        ...
        <div class=" col-md-6 offset-md-3" style="padding-left: 7px;">
            <button type="submit" class="btn btn-primary btn-sm">
                <i class="fa fa-dot-circle-o"></i> {{isset($users) ? 'Update users' : 'Add New'}}
            </button>
        </div>
