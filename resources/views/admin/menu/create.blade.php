...
<div class="col-lg-12">
    <div class="card">
        ...
        <div class="card-body card-block">
            <form action="{{isset($categories) ? route('categories.update', $categories->id) : route('categories.store')}}" method="POST" enctype="multipart/form-data">
                @csrf
                @if(isset($categories))
                @method('PUT')
                @endif
        ...
        <div class=" col-md-6 offset-md-3" style="padding-left: 7px;">
            <button type="submit" class="btn btn-primary btn-sm">
                <i class="fa fa-dot-circle-o"></i> {{isset($categories) ? 'Update categories' : 'Add New'}}
            </button>
        </div>
